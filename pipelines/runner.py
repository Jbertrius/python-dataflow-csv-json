...
def run(argv=None, save_main_session=True):
    ...
    # Merge PubMed
    pubmed = (p | 'Read from a File Pub' >> ReadFromText(known_args.input_pubmed, skip_header_lines=1)
              | 'Map pub to json' >> beam.Map(lambda s: data_ingestion_pubmed.parse_method(s))
              | 'detect drug in publication' >> beam.FlatMap(create_key_value_pair, AsList(drugs_as_list), 'title')
              )

    pubmedv2 = (p | 'read_data' >> beam.Create(json.loads(FileSystems.open(known_args.input_pubmed_json).read()))
                | 'map' >> beam.FlatMap(create_key_value_pair, AsList(drugs_as_list), 'title'))

    merged_pubmed = ((pubmed, pubmedv2) | 'Merge PCollections' >> beam.Flatten()
                     | beam.Filter(lambda x: x))
